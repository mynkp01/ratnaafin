...
const Page = () => {
  ...
  const fetchCity = async (currentPage, keyword = "") => {
    try {
      ...
      const { data, status } = await apiHandler.country.list(query);
  ...
  const handleView = (country) => {
    window.open(`${ROUTES.admin.country}/option?id=${country._id}&view=1`, "_blank", "noopener,noreferrer");

    // router.push(`${ROUTES.admin.country}/option?id=${country._id}&view=1`);
  };

  const handleUpdate = (country) => {
    window.open(`${ROUTES.admin.country}/option?id=${country._id}`, "_blank", "noopener,noreferrer");

    // router.push(`${ROUTES.admin.country}/option?id=${country._id}`);
  };
  ...
  return (
    <div className="border-wh-300 flex flex-col !gap-4 rounded-2xl border bg-white !p-4 md:!p-6">
      <div className="flex justify-between">
        <h1 className="text-xl font-bold">Manage Countries</h1>
        <button
          onClick={
            () => window.open(`${ROUTES.admin.country}/option`, "_blank", "noopener,noreferrer")
            // router.push(`${ROUTES.admin.country}/option`)
          }
